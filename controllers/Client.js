...
exports.createClients = async (req, res) => {
  try {
    ...
    for (let i = 0; i < find_client.length; i++) {
      var index = emails.findIndex(
        (x) => x.client_email == find_client[i].client_email
      );
      if (index != -1) {
    ...
    await Client.bulkCreate(payload)
      .then(async (resp) => {
...
exports.updateClient = async (req, res) => {
  try {
    const { client } = req.body;
    var update_client = await Client.update(client, {
      where: {
        id: client.id,
      },
    });
    ...
    if (!find_otp) {
      ...
      if (find_otp.expireAt <= time) {
        return res.status(200).json({
          ...
          isExpired: find_otp.expireAt <= time,
        });
      } else {
...
exports.changePassword = async (req, res) => {
  try {
    ...
    await bcrypt
      .compare(client.old_password, dbPassword)
      .then(async (match) => {
        if (!match) {
          return res.status(200).json({
            error: "Wrong Credential!",
...
exports.createSign = async (req, res) => {
  try {
    ...
    var createSign;
    if (!get_Sign) {
      createSign = await UserSign.create(signiture);
      if (!createSign) {
        return res.status(400).json({
      ...
      createSign = await UserSign.update(signiture, {
        where: { signiture: signiture.signiture },
      });
      return res.status(200).send({
        message: "created",
