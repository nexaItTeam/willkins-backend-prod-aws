...
exports.updateProperty = async (req, res) => {
  try {
    const { property } = req.body;
    var updateProperty = await Property.update(property, {
      where: {
        id: property.id,
      },
    });
...
exports.uploadeBrochure = async (req, res) => {
  try {
    ...
    await azureUpload(file)
      .then(async (resp) => {
        var brochure = await db.sequelize.query(
          `UPDATE property SET brocher='${file.originalname}' WHERE id = ${id};`
        );
        res.status(200).json({
          message: "Success upload",
          brochure,
...
exports.uploadespds = async (req, res) => {
  try {
    ...
    await azureUpload(file)
      .then(async (resp) => {
        var spds = await db.sequelize.query(
          `UPDATE property SET brocher='${file.originalname}' WHERE id = ${id};`
        );
        res.status(200).json({
          message: "Success upload",
...
exports.uploadestdm = async (req, res) => {
  try {
    ...
    await azureUpload(file)
      .then(async (resp) => {
        var stdm = await db.sequelize.query(
          `UPDATE property SET brocher='${file.originalname}' WHERE id = ${id};`
        );
        res.status(200).json({
          message: "Success upload",
...
exports.uploadesfsg = async (req, res) => {
  try {
    ...
    await azureUpload(file)
      .then(async (resp) => {
        var fsg = await db.sequelize.query(
          `UPDATE property SET brocher='${file.originalname}' WHERE id = ${id};`
        );
        res.status(200).json({
          message: "Success upload",
...
exports.total_investment = async (req, res) => {
  try {
    const totalInvestment = await db.sequelize.query(
      `SELECT sum(investing_amount) as total_investment  FROM nexa_capital.order where prop_id= ${req.body.prop_id} and paidStatus != 3 and paidStatus !=0 and holder_type='self'; `
    );
    if (totalInvestment) {
      res.status(200).json({
        message: "Success upload",
...
exports.getAllPropertyMobile = async (req, res) => {
  try {
    const images = await PropertyIMG.findAll();
